...
(function() {
  var FALSE, Franz, TRUE, byArray, byImage, cbr, clamp, copy, crop, dlog, fff, fffr, getGrayscaleValue, getToolbox, hardResize, ifw, isFunction, mF, make, nb, newToolbox, resize, rgba, scale, toArray, toImage,
    __slice = [].slice;
  ...
  Franz.preserveYellow = function() {
    var bt, c, cb, gt, p, rt, _ref;
    c = arguments[0], p = 2 <= arguments.length ? __slice.call(arguments, 1) : [];
    _ref = fff(p, 127, 127, 50), cb = _ref[0], rt = _ref[1], gt = _ref[2], bt = _ref[3];
    return Franz.applyIfRgba(c, Franz.blackWhite, [], (function(r, g, b, a, i) {
      if (r > rt && g > gt && b < bt) {
        return false;
      } else {
  ...
  Franz.c64 = function(c, cb) {
    return Franz.reduceAndReplace(c, cb);
  };

  Franz.ascii = function() {
    var asciiA, ascii_string, c, cb, nr_of_buckets, p, _ref;
    c = arguments[0], p = 2 <= arguments.length ? __slice.call(arguments, 1) : [];
    _ref = fff(p, '@GLftli;:,.  '), cb = _ref[0], ascii_string = _ref[1];
    asciiA = asciistring.split('');
    return nr_of_buckets = asciiA.length;
  };
